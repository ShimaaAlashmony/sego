...
const Company = () => {
  ...
  return (
    <div className="container flex flex-col items-center justify-center gap-8 mx-auto sectionPadding pb-16">
      ...
      <div className="w-[150px] h-[150px] overflow-hidden rounded-full mr-[84%]">
        <Image
          src={data?.logo}
      ...
      <p className="text-center text-2xl text-[#1c3383] -ml-[84%]">{data?.name}</p>
      {/* <button onClick={() => setOpenUpdateCompany(true)}><EditIcon /></button> */}
      <div>
         <h3 className=" text-lg text-[#1f253a] mb-[25px]">{data?.overview}</h3>
        {/* <div className="flex items-center my-8 gap-x-4">
          <PhoneIcon />
          <span>{data?.social?.[0]?.whatsapp}</span>
        </div> */}
        <div className='flex items-center gap-x-2 text-[#1f253a] hover:text-[#5e66b6] '>
          <MailIcon />
